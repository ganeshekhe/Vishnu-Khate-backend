...
router.post("/draft", verifyToken, async (req, res) => {
  try {
    ...
    const populated = await Application.findById(draft._id)
      .populate("user", "name mobile caste dob")
      .populate({ path: "service", select: "name category", populate: { path: "category", select: "name" } })
      .populate("subService", "name");

    req.io.emit("applicationDrafted", populated);
    res.status(201).json({ message: "Draft created", application: populated });
  } catch (err) {
    console.error("Draft create error:", err.message);
...
router.post("/", verifyToken, async (req, res) => {
  try {
    const { serviceId, subServiceId, userId } = req.body;
    if (!serviceId || !userId) return res.status(400).json({ message: "Service and User ID required" });
    ...
    if (subServiceId) {
      ...
      if (selectedSub) subServiceData = { _id: selectedSub._id, name: selectedSub.name };
    }
...
// ---------- Upload Form PDF + Operator Credentials ----------
router.put("/:id/upload-pdf", verifyToken, requireRole("operator"), upload.single("formPdf"), async (req, res) => {
  try {
    const { operatorId, operatorPassword } = req.body;
    const app = await Application.findById(req.params.id);
    if (!app) return res.status(404).json({ message: "Application not found" });
    ...
    req.io.emit("applicationStatusUpdated", populated);
    res.json({ message: "PDF + credentials uploaded", application: populated });
  } catch (err) {
    console.error("Upload failed:", err);
    res.status(500).json({ message: "Upload failed", error: err.message });
  }
...
router.put("/:id/confirm", verifyToken, async (req, res) => {
  try {
    if (req.user.role !== "user") return res.status(403).json({ message: "Only user can confirm" });

    const app = await Application.findById(req.params.id);
    if (!app) return res.status(404).json({ message: "Application not found" });
    if (app.status !== "Pending Confirmation") return res.status(400).json({ message: "Not ready for confirmation" });
    ...
    res.status(500).json({ message: "Failed to confirm", error: err.message });
  }
});
...
router.put("/:id/correction", verifyToken, async (req, res) => {
  try {
    const { comment } = req.body;
    if (!comment || !comment.trim()) return res.status(400).json({ message: "Correction comment required" });

    const app = await Application.findById(req.params.id);
    if (!app) return res.status(404).json({ message: "Application not found" });
    if (app.user.toString() !== req.user.id) return res.status(403).json({ message: "Not authorized" });
    ...
    res.status(500).json({ message: "Correction failed", error: err.message });
  }
});

// ---------- Operator/Admin Next Step ----------
router.put("/:id/next", verifyToken, requireRole("operator", "admin"), async (req, res) => {
  try {
    const app = await Application.findById(req.params.id);
    if (!app) return res.status(404).json({ message: "Application not found" });
    ...
    const newStatus = nextStatusOf(app.status);
    if (!newStatus) return res.status(400).json({ message: `No next step from ${app.status}` });
...
router.put("/:id/uploadCertificate", verifyToken, requireRole("operator", "admin"), upload.single("certificate"), async (req, res) => {
  try {
    ...
    if (!["Confirmed", "Waiting"].includes(app.status))
      return res.status(400).json({ message: "Certificate upload allowed only after confirmation or waiting" });
...
// ---------- User updates rejected application ----------
router.put("/:id/update", verifyToken, async (req, res) => {
  try {
    if (req.user.role !== "user") return res.status(403).json({ message: "Only user can update" });

    const app = await Application.findById(req.params.id);
    if (!app) return res.status(404).json({ message: "Application not found" });
    if (app.user.toString() !== req.user.id) return res.status(403).json({ message: "Not authorized" });
    if (app.status !== "Rejected") return res.status(400).json({ message: "Only rejected applications can be updated" });

    const { updates } = req.body;
    if (updates) {
      if (updates.data) app.data = { ...app.data, ...updates.data };
      if (updates.documents) app.documents = updates.documents;
      if (updates.correctionComment) app.correctionComment = updates.correctionComment;
    }
    ...
    req.io.emit("applicationStatusUpdated", populated);
    res.json({ message: "Application updated & resubmitted", application: populated });
  } catch (err) {
    res.status(500).json({ message: "Update failed", error: err.message });
  }
});
...
router.get("/", verifyToken, async (req, res) => {
  try {
    if (!["admin", "operator"].includes(req.user.role)) return res.status(403).json({ message: "Access denied" });
